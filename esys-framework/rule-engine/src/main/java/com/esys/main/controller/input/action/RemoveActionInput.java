...
@AllArgsConstructor
@NoArgsConstructor
public class RemoveActionInput {
		...
		public static RengineAction toEntity(RemoveActionInput input) {
			RengineAction entity = new RengineAction();
		...
		public static RemoveActionInput toDTO(final RengineAction entity){
			RemoveActionInput dto=new  RemoveActionInput();
		...
		public static List<RemoveActionInput> toDTOList(final List<RengineAction> entityList){
			List<RemoveActionInput> dtoList=new ArrayList<RemoveActionInput>(0);
			if(entityList!=null) {
				for(RengineAction entity:entityList) {
					RemoveActionInput dto=RemoveActionInput.toDTO(entity);
					dtoList.add(dto);
				}
			}
			return dtoList;
		...
		public static List<RengineAction> toEntityList(final List<RemoveActionInput> dtoList){
			List<RengineAction> entityList=new ArrayList<RengineAction>(0);
			if(entityList!=null) {
				for(RemoveActionInput dto:dtoList) {
